...
| **Projections and caches** — `findings.db` (the traust-contracts storage/v1 store on SQLite), the portfolio graph, the security-feed cache, remote materialisations | beside the findings store, or remote | `locations.yaml` → `portfolio_graph` (defaults to `<analysis_results>/graph/portfolio-graph.db`); `locations.yaml` → `feeds_cache` (**no default** — unset, feed fetch fails closed); `HARNESS_REMOTE_CACHE` | Regenerable. Deleting any of them loses nothing; the drift checker reports when one is stale. `findings.db` is never a write target; a PostgreSQL adopter gets the same views from the database instead. |
